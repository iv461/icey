...
int main(int argc, char **argv) {
  ...
  /// header stamp. It will wait up to 200ms for the transform. (currently uses
  /// tf2_ros::MessageFilter for this)
  node->icey()
      ...
      .unwrap_or([&](std::string error) {
        /// TODO [ROS limitation]: Currently this is never called because we can't register an error
        /// callback when using the tf2_ros::MessageFilter (it will still print an error, but we
        /// can't register a custom callback)
      ...
      .then([&](sensor_msgs::msg::PointCloud2::SharedPtr point_cloud,
        ...
        RCLCPP_INFO_STREAM(
            node->get_logger(),
            "Received point cloud with header time: "
                << fmt::format("{:%H:%M:%S}\n", icey::rclcpp_to_chrono(point_cloud->header.stamp))
                << "\nAnd transform at time: "
                << fmt::format("{:%H:%M:%S}\n",
