/// This example shows how multiple nodes can be created and launched in a single process in
/// (promise style).
...
int main(int argc, char **argv) {
  ...
  received_sine_signal.then([&](std_msgs::msg::Float32::SharedPtr signal_value) {
    RCLCPP_INFO_STREAM(receiver_node->get_logger(),
                       "Received the sine signal: " << signal_value->data);
  });
