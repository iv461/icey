...
TEST_F(NodeTest, SyncApproxTimeTest) {
  ...
  auto synched = node_->icey()
                     .synchronize_approx_time(5, images, point_clouds)
...
TEST_F(NodeTest, AnyTest) {
  ...
  auto s2 = node_->icey().create_timer(53ms).then([](size_t ticks) -> std::optional<std::string> {
    if (ticks >= 3) return {};
    return "s2_" + std::to_string(ticks + 1);
  });

  auto s3 = node_->icey().create_timer(67ms).then([](size_t ticks) -> std::optional<std::string> {
    if (ticks >= 3) return {};
...
TEST_F(NodeTest, UnpackTest) {
  ...
  auto [double_stream, tf_stream2, string_stream] =
      timer
          .then([&](size_t ticks) {
            auto val = std::make_tuple(double(ticks), geometry_msgs::msg::TransformStamped{},
                                       std::string{});
            if (ticks >= 3) {
  ...
  double_stream.then([&](double) { double_stream_num_called++; });

  tf_stream2.then([&](geometry_msgs::msg::TransformStamped) { tf_stream2_num_called++; });

  string_stream.then([&](std::string) { string_stream_num_called++; });
...
TEST_F(NodeTest, FilterTest) {
  ...
  timer.filter([&](size_t ticks) { return !(ticks % 3); }).then([&](size_t ticks) {
    if (ticks > 10) {
      timer.impl()->timer->cancel();
      return;
    }
